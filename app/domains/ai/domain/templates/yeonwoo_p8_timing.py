...
PARA_2_TEMPLATE = (
    "이번 달부터 1년을 봐. 너의 연줄에서 실이 가장 강하게 당겨지는 건 "
    "{peak_1}과 {peak_2}이야. 그 두 달은 가만히 있어도 사람이 들어와. "
    "그 사이는 충전 구간이야. 조급해하지 마. 비우는 시간이 곧 채우는 시간이야. 없으면 네가 더 힘들어져."
)
...
ILGAN_FLOW_BY_ILGAN: dict[str, str] = {
    "갑목": (
        "{ilgan_with_hanja} 일간은 곧게 가는 형국이야. "
        "흐름이 막혀도 직진하는 게 좋아. 피크에 한 방향으로 가는 게 너한테 제일 잘 맞아."
    ),
    "을목": (
        "{ilgan_with_hanja} 일간은 휘어 살아남는 느낌이야. "
        "흐름에 맞춰 유연하게 구는 게 좋지만, 한 번 감기면 끝까지 봐. 그게 너한테 제일 잘 맞아."
    ),
    "병화": (
        "{ilgan_with_hanja} 일간은 빛으로 사람을 끄는 매력이야. "
        "피크에 빨리 표현하는 게 좋아. 식기 전에 상대에게 전하는 게 너한테 가장 잘 맞아."
    ),
    "정화": (
        "{ilgan_with_hanja} 일간은 한 사람만 비추는 사람이야. "
        "작은 불빛이지만 꾸준히 오래 비추는 게 너한테 맞아. 피크엔 조용히 깊게 다가는 게 좋아."
    ),
    "무토": (
        "{ilgan_with_hanja} 일간은 묵직하게 받쳐주는 사람이야. "
        "흔들리지 말고 그 자리에 있어줘. 그래도 피크엔 한 걸음 다가가는 게 맞을 거야."
    ),
    "기토": (
        "{ilgan_with_hanja} 일간은 다정하게 받쳐주는 사람이야. "
        "피크엔 곁을 내주고 표현을 해주는 게 너한테 잘 맞을 거야."
    ),
    "경금": (
        "{ilgan_with_hanja} 일간은 결단력 있는 사람이야. "
        "피크엔 분명하게 한마디를 던지는 게 좋아. 다만, 기억하지? 너무 벼린 말은 하지 마."
    ),
    "신금": (
        "{ilgan_with_hanja} 일간은 다듬어진 사람이야. "
        "피크에도 결을 흐트리지 마. 정갈하게 행동하던 거 그대로 유지하는 게 좋아."
    ),
    "임수": (
        "{ilgan_with_hanja} 일간은 흐름을 거스르면 안 되는 사람이야. "
        "피크에 움직이고, 정체기엔 멈춰서 주변을 살펴봐. 그게 너에게 가장 잘 맞아."
    ),
    "계수": (
        "{ilgan_with_hanja} 일간은 분위기로 사랑하는 사람이야. "
        "흐름 따라 스며들지. 피크엔 분위기로 신호를 주는 게 너에게 잘 맞아."
    ),
}
...
STATE_DESC_BY_STATE: dict[str, str] = {
    "시작":        "이번 달부터 흐름이 움직여. 큰 사건은 없어도 주변이 바뀌는 시기야.",
...
def _format_month_label(idx: int, start_year: int, start_month: int) -> str:
    """idx=0 → '5월 (이번달)' / 같은 해 → '6월' / 다음 해 → "27년 1월"."""
    y = start_year
    m = start_month + idx
    if m > 12:
        m -= 12
...
def _peak_label_for_ai(idx: int, start_year: int, start_month: int) -> str:
    """AI 박스 안 '8월/ 27년 1월' 같이 (이번달) 표기 없이 단순화."""
    y = start_year
    m = start_month + idx
    if m > 12:
        m -= 12
